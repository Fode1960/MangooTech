...
const readMockBoutiques = (): any[] => {
  try {
    const raw = localStorage.getItem('mock_boutiques');
    const parsed = raw ? JSON.parse(raw) : [];
    return Array.isArray(parsed) ? parsed : [];
  } catch {
    return [];
...
export default function AdminShops() {
    ...
    void (async () => {
      ...
      try {
        ...
        if (res.ok && json?.success && Array.isArray(json?.shops)) {
          ...
          supabaseMapped = mapped
        }
      } catch {
      }

      if (!supabaseMapped.length && canUseSupabase) {
        try {
          const { data, error } = await supabase
            .from('shops')
            .select('id,name,slug,category,status,owner_name,owner_email,email,approved_at,approved_by,rejected_at,rejected_by,suspended_at,suspended_by,created_at,updated_at,shop_url')
    ...
    if (mockBoutiques.length) {
      ...
      mockBoutiques.forEach((m: any) => {
        ...
        const nextShop: DemoShop = {
          ...
          slug,
          category: String(m?.category || (Array.isArray(m?.categories) ? m.categories[0] : '') || 'general'),
          ownerName: String(m?.owner_name || ''),
          ownerEmail: String(m?.owner_email || m?.email || ''),
          approvalStatus,
          source: 'mock',
          updatedAt: String(m?.updated_at || m?.updatedAt || now),
          createdAt: String(m?.created_at || m?.createdAt || now),
          sourceVendorId: String(m?.id || ''),
        };
  ...
  const setApproval = useCallback(async (slug: string, status: ApprovalStatus) => {
    const targetShop = shops.find((s) => String(s?.slug || '').trim() === String(slug || '').trim()) as any
    const targetId = String(targetShop?.id || '').trim()
    const targetSource = String(targetShop?.source || '').trim()
    const isLocalSync =
      targetSource === 'local-sync' ||
      targetId.startsWith('local_') ||
      targetId.startsWith('s_')
    const localSyncId =
      targetId.startsWith('local_')
        ? targetId.replace(/^local_/, '')
        : targetId.startsWith('s_')
          ? targetId
          : ''

    const pushLocalSyncStatus = async (shopId: string) => {
      const mapped = status === 'approved' || status === 'rejected' ? status : 'pending'
      const resp = await fetch(`/api/local-sync/admin/shops/${encodeURIComponent(String(shopId))}`, {
        method: 'PATCH',
    ...
    if (isLocalSync && localSyncId) {
      ...
      try {
        await pushLocalSyncStatus(localSyncId)
        toast.success('Statut mis à jour')
      } catch (e: any) {
        toast.error(`Impossible de mettre à jour (Local Sync): ${String(e?.message || 'Erreur')}`)
      } finally {
        refresh()
      }
      return
    ...
    if (targetSource === 'supabase' && canUseSupabase) {
      setShops((prev) => prev.map((s) => {
        if (String(s?.slug || '') !== slug) return s
        const now0 = new Date().toISOString()
        if (status === 'approved') return { ...s, approvalStatus: 'approved', approvedAt: now0, approvedBy: ADMIN_EMAIL, updatedAt: now0 }
