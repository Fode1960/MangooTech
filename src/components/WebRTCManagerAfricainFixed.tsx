...
import { Phone, PhoneOff, Video, VideoOff, Mic, MicOff, Send, User, Bell } from 'lucide-react';

interface WebRTCManagerAfricainFixedProps {
...
const WebRTCManagerAfricainFixed: React.FC<WebRTCManagerAfricainFixedProps> = ({
  mode,
  roomId,
  userRole,
  onStreamStart,
  ...
  const playRingtone = () => {
    try {
      ...
      oscillator.frequency.setValueAtTime(800, audioContextRef.current.currentTime + 0.4);
      ...
      ringtoneRef.current = oscillator;
  ...
  const startCall = async () => {
    try {
      ...
      // Envoyer notification d'appel AVANT de créer l'offre
      if (userRole === 'vendor') {
        sendCallNotification();
        // Attendre un peu pour que la notification soit reçue
    ...
    <div className="h-screen bg-gradient-to-br from-orange-50 to-amber-50 flex flex-col overflow-hidden">
      ...
      {incomingCall && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 text-center max-w-md mx-4 shadow-2xl animate-pulse">
      ...
      {/* Zone principale avec hauteur fixe */}
...
export default WebRTCManagerAfricainFixed;