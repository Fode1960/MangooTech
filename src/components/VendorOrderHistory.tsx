...
function buildInvoiceOrder(order: Order): VendorOrder {
  return {
    ...
    paymentMethod: String(order?.payment?.provider || 'Paiement confirm�'),
      ...
      <div className="space-y-4">
            ...
            <div key={order.id} className={`rounded-xl shadow-lg p-6 ${isDark ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                    <div>
                      ...
                      <div className="flex items-center space-x-2 mb-1">
                        ...
                        <span className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{order.customerAddress || 'Adresse client � compl�ter'}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <DollarSign className={`h-4 w-4 ${isDark ? 'text-gray-400' : 'text-gray-500'}`} />
                        <span className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{String(order?.payment?.provider || 'Paiement confirm�')}</span>
                      </div>
                    </div>
                    <div>
                      <div className="flex items-center space-x-2 mb-1">
                ...
                <div className="flex flex-col sm:flex-row items-end gap-2">
                  ...
                  >
                    {deliveryLaunched ? '? Livraison lanc�e' : '?? Colis pr�t -> Lancer la livraison'}
                  </button>
                </div>
              </div>
            </div>
        ...
        {filteredOrders.length === 0 && (
          <div className={`text-center py-8 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
            ...
            <p>Aucune commande vendeur trouv�e</p>
            <p className="text-sm mt-2">Les commandes pay�es apparaissent ici pour lancer la livraison c�t� vendeur.</p>
          </div>
        )}
      </div>
          ...
          <div className={`rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto ${isDark ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h3 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>D�tails de la commande {selectedOrder.id}</h3>
                <button
              ...
              <div className="space-y-4">
                <div className={`p-4 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
                  ...
                  <p className={isDark ? 'text-gray-300' : 'text-gray-700'}><strong>T�l�phone :</strong> {selectedOrder.customerPhone || '�'}</p>
                  <p className={isDark ? 'text-gray-300' : 'text-gray-700'}><strong>Adresse :</strong> {selectedOrder.customerAddress || '�'}</p>
                </div>
                <div className={`p-4 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
                  <h4 className={`font-semibold mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>D�tails</h4>
                  ...
                  <p className={isDark ? 'text-gray-300' : 'text-gray-700'}><strong>Paiement :</strong> {String(selectedOrder?.payment?.provider || 'Paiement confirm�')}</p>
