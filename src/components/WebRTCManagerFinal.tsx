...
const WebRTCManagerFinal: React.FC<WebRTCManagerFinalProps> = ({ 
  ...
  // Connexion WebSocket
  useEffect(() => {
    const ws = new WebSocket('ws://localhost:3008'); // Utiliser le nouveau port WebRTC
    wsRef.current = ws;
  ...
  const startCall = async () => {
    ...
    try {
      ...
      // Obtenir le flux média avec configuration audio OPTIMISÉE POUR LA VOIX
      const stream = await navigator.mediaDevices.getUserMedia({ 
        ...
        audio: {
          ...
          autoGainControl: true,
          sampleRate: 48000,
          channelCount: 2, // STEREO pour meilleure qualité vocale
          volume: 1.0, // Volume MAXIMUM pour la voix
          // Configuration supplémentaire pour la voix claire
          googEchoCancellation: true,
          googNoiseSuppression: true,
          googAutoGainControl: true,
          googHighpassFilter: true,
  ...
  const answerCall = async () => {
    ...
    try {
      ...
      // Vérifier et fermer toute connexion existante
      if (peerConnectionRef.current) {
        console.log('🔄 Fermeture de la connexion WebRTC existante côté répondeur...');
        peerConnectionRef.current.close();
        peerConnectionRef.current = null;
      }
      ...
      stream.getTracks().forEach(track => {
        console.log('🎵 Ajout track côté répondeur:', track.kind, track.label);
        pc.addTrack(track, stream);
      });
      
      console.log('✅ Tracks ajoutés à la connexion WebRTC côté répondeur');
  ...
  const handleOffer = async (offer: RTCSessionDescriptionInit) => {
    try {
      ...
      if (!localStreamRef.current) {
        ...
        // Créer le flux local pour le répondeur
        const stream = await navigator.mediaDevices.getUserMedia({ 
          video: isVideoEnabled, 
          audio: {
            echoCancellation: true,
    ...
    try {
      if (peerConnectionRef.current) {
        ...
        if (pc.remoteDescription) {
          ...
          // Stocker le candidat pour l'ajouter après la réponse
          setTimeout(() => {
            if (peerConnectionRef.current && peerConnectionRef.current.remoteDescription) {
              peerConnectionRef.current.addIceCandidate(new RTCIceCandidate(candidate))
                .then(() => console.log('✅ Candidat ICE ajouté en différé'))
                .catch(err => console.error('❌ Erreur ajout candidat ICE différé:', err));
            }
          }, 1000);
        }
      }
