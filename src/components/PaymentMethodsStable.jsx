...
import { ensureWalletBalance, getWalletBalance, creditWalletBalance, getWalletKeyFromUser } from '../utils/demoWallet';
...
export const PaymentMethods = ({ 
  ...
  const walletKey = useMemo(() => {
    ...
    const fromLocalStorage =
      parse(localStorage.getItem('mangoo-current-user')) ||
      parse(localStorage.getItem('user')) ||
      parse(localStorage.getItem('local_user')) ||
      parse(localStorage.getItem('test_user')) ||
      parse(localStorage.getItem('demo_user'));
    return getWalletKeyFromUser(fromLocalStorage, userId) || String(userId || '').trim() || 'demo-user';
  }, [userId]);
  ...
  const creditWallet = useCallback((creditAmount) => {
    creditWalletBalance(walletKey, creditAmount);
  }, [walletKey]);

  useEffect(() => {
    const onWallet = () => refreshWalletBalance();
  ...
  }, [creditWallet, email, phoneNumber, processPayment, refreshWalletBalance, walletKey, walletTopupChannel, walletTopupMethod, walletTopupAmount, walletTopupReference]);
