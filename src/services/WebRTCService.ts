...
import { getHttpUrl, getWsUrl } from '../utils/realtimeUrls';
...
class WebRTCService {
  ...
  private async fetchRTCConfig(): Promise<void> {
    try {
      const response = await fetch(`${getHttpUrl(8081)}/api/turn/config?username=${this.userId}&role=${this.userRole}`);
  ...
  private connectToSignalingServer(): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        this.signalingSocket = new WebSocket(getWsUrl(8080));
