...
import { ensureWalletBalance, debitWalletBalance, creditWalletBalance, getWalletKeyFromUser } from '../utils/demoWallet.js';
...
export const usePaymentStore = create((set, get) => ({
  ...
  processPayment: async (paymentData) => {
    ...
    const resolveWalletKey = () => {
      const fallback = String(paymentData?.userId || '').trim() || String(paymentData?.email || '').trim() || String(email || '').trim();
      return getWalletKeyFromUser({ id: paymentData?.userId, email: paymentData?.email || email }, fallback) || fallback;
    };
        ...
        try {
          ...
          if (Number.isFinite(creditAmount) && creditAmount > 0) {
            try {
              ...
              if (walletKey) creditWalletBalance(walletKey, creditAmount);
            } catch {
            }
          }
