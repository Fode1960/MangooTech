...
class MiniBoutiqueAutoStarter {
  ...
  async startMiniBoutique() {
    ...
    try {
      ...
      const packageJsonPath = path.join(this.miniBoutiquePath, 'package.json');
      const fs = require('fs');
...
module.exports = MiniBoutiqueAutoStarter;

// Démarrage automatique si ce script est exécuté directement
if (require.main === module) {
  const starter = new MiniBoutiqueAutoStarter();
  
  console.log('🚀 Système de démarrage automatique Mini-Boutique activé');
  
  // Démarrer Mini-Boutique immédiatement
  starter.startMiniBoutique().then((success) => {
    if (success) {
      console.log('✅ Mini-Boutique démarré automatiquement');
      starter.startMonitoring();
    } else {
      console.error('❌ Échec du démarrage automatique de Mini-Boutique');
      process.exit(1);
    }
  });
}