...
const isLocalShopId = (id) => id.startsWith('local-') && id.includes('@')
...
export default async function handler(req, res) {
  try {
    ...
    const input = Array.isArray(r?.data) ? r.data : []
    ...
    const emailToShopId = new Map()
    if (emails.length) {
      try {
        const s = await supabase.from('shops').select('id,email').in('email', emails)
