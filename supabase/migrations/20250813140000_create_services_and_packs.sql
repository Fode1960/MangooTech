...
CREATE POLICY "Users can insert their own packs" ON user_packs FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own packs" ON user_packs FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Service role can manage user packs" ON user_packs FOR ALL USING (auth.jwt() ->> 'role' = 'service_role');
