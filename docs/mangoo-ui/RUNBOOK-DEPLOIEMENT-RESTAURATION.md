...
**Commande boutique (résolu).** La fiche boutique (`fiche-boutique.html`) crée désormais une commande serveur AVANT le paiement via `POST /api/boutique/order` (persistée dans `boutique-orders.json`, commande `en_attente`). Au retour du paiement live, la branche `boutique-order` du settlement marque la commande `payee`, lie le `transactionId`, décrémente le stock des articles du catalogue et crédite le vendeur net de commission (`settlementAmount`/`commissionAmount`). L'idempotence reste assurée par `txn.settled` et le garde-fou `status !== 'payee'`.
