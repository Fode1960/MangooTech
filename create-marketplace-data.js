...
async function createMarketplaceTestData() {
  try {
    ...
    // 2. Créer une boutique de test (nécessite un utilisateur)
    console.log('🏪 Création de la boutique de test...')
    
    // Obtenir le premier utilisateur disponible
    ...
    if (usersError || !users || users.length === 0) {
      console.log('⚠️ Aucun utilisateur trouvé. Création d\'une boutique sans utilisateur...')
    }
    ...
    const products = [
      {
        shop_id: shop.id,
        category_id: (await supabase.from('categories').select('id').eq('slug', 'electronique').single()).data?.id,
      ...
      {
        shop_id: shop.id,
        category_id: (await supabase.from('categories').select('id').eq('slug', 'mode').single()).data?.id,
      ...
      {
        shop_id: shop.id,
        category_id: (await supabase.from('categories').select('id').eq('slug', 'maison').single()).data?.id,
